...
* **永世留存的歷史快照與精準溯源**：打破底層排程引擎會將「過期/僅一次」排程從資料庫抹除的限制，本系統會動態攔截並將排程的**原始 JSON 參數快照**永久封裝進資料庫。更深度區分了「使用者手動觸發」與「排程器自動執行」的日誌標籤；即使遇到衝突，狀態轉譯也極具指標性 (如：「依並發規則略過」)。未來無論如何修改排程設定，歷史軌跡永遠忠實保留！而前端日誌面板更加入了 7 日強制過濾防護機制，在享受無限期溯源的同時保障系統極致的讀取與渲染效能。
...
* **僅一次**：可自定一次性任務，完美映射 SimpleTrigger。
* **定期循環**：全新專用觸發模式！無懼跨夜與午夜重置時間區間限制，指定首輪發動起點後完美達成 24/7 永恆定期間隔輪迴。
* **每天**：嚴格框定每日營業時間 (如 08:00~23:59)，可自訂每隔 X 天絕對循環執行一次。
...
若要進行人員交接、功能講解、維運查錯或後續開發，請從以下文件開始：

* [交接文件入口](docs/00_交接文件入口.md)
* [前端使用者操作手冊](docs/01_前端使用者操作手冊.md)
* [後端開發實作說明](docs/02_後端開發實作說明.md)
* [部署維運與測試手冊](docs/03_部署維運與測試手冊.md)
* [關鍵字索引](docs/04_關鍵字索引.md)
